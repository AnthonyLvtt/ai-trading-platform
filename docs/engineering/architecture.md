...
Backtesting V1 consumes accepted DATA, Strategy, and Risk artefacts and is the only simulated-execution boundary. An approved economic intention creates a quantity-free simulated order at the evaluation bar and may fill only at the open of the first admissible next bar for the same symbol. Non-approved Risk results never create orders. The replay owns only an `EMPTY` or `OPEN_LONG(symbol)` structural state and produces no Accounting, PnL, fee, slippage, OMS, Exchange, network, AI/ML, Testnet, or Live behaviour.

Accounting V1 consumes an existing `SimulatedFill` plus an explicit external quantity fact. It records deterministic economic transitions in an immutable in-memory ledger and values an open position only from an admissible causal DATA close mark. Accounting never selects quantity, creates an order or fill, contacts an Exchange, or owns persistence.

Observability V1 maps accepted domain artefacts to typed, versioned event references without gaining business authority. Events form a deterministic, immutable in-memory audit chain with explicit correlation, causation and UTC logical time. Diagnostic logs remain separate, non-authoritative and sanitized. Observability has no remote telemetry backend, persistence, network, OMS, Exchange or AI/ML dependency.
